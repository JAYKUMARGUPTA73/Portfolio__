  ...
  return (
    <motion.header
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-[#0a192f] shadow-lg' : 'bg-transparent'
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
        <div className="flex items-center">
          <Link to="hero" smooth duration={500}>
            <h1 className="text-2xl ntr-regular font-bold text-accent cursor-pointer mr-6">Gazi Jarin</h1>
          </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden px-6 py-3 bg-gray-900">
          <ul className="flex flex-col space-y-4 text-accent font-medium">
            {['Home', 'About', 'Experience', 'Projects'].map((item) => (
              <li key={item}>
                <Link
                  to={item.toLowerCase()}
